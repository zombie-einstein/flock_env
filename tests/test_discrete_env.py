...
def test_environment_reset(params: Dict):
    ...
    for _ in range(100):
        obs = env.reset()
        assert obs.shape == (params["n_agents"], 3*params["n_nearest"]+(2*n_obstacles))
...
def test_agent_update(params: Dict):
    ...
    env.x[:, :n_agents] = np.array([[0.5 for _ in range(n_agents)],
                                    [0.5 for _ in range(n_agents)]]).astype(np.float32)

    for _ in range(10):
        old_x = env.x.copy()
        env._update_agents()
        assert np.allclose(env.x[0, :n_agents], old_x[0, :n_agents] + env.speed*params["speed"])
