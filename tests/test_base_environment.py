...
def dummy_env():
    ...
    class DummyEnv(BaseFlockEnv):
        ...
        def get_obs(
            self, state: flock_env.EnvState, params: flock_env.EnvParams
        ) -> chex.Array:
...
def test_update_sequence(params: flock_env.EnvParams, env: BaseFlockEnv):
    ...
    _, state_0 = env.reset_env(key, params)
    ...
    assert jnp.array_equal(state_seq.time, jnp.arange(n_steps) + 1)
...
def test_movement(dummy_env):
    ...
    def step(s, _):
        _, new_state, _, _, _ = test_env.step_env(k, s, actions, p)
